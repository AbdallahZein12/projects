...
import win32api
import win32con
from chatgpt.global_variables import global_music_box, loading_screen  
...
class main:
    def __init__(self):
        ...
        self.login_button = tk.Button(self.root,text="Login",width=20,background="Magenta",font=("Bauhaus 93",18),command=self.start_login_task)
    ...
    def start_login_task(self):
        loader = loading_screen(self.root)
        threading.Thread(target=self.default_login, args=(loader,)).start()
        
    def default_login(self,loader):
        ...
        try:
            ...
            messagebox.showinfo(title="Result",message=default_login)
            loader.close()
        except Exception as e:
            messagebox.showinfo(title="An error has occured!",message=e)
            loader.close()
